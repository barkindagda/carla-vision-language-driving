...
class VLMRolloutBuffer(RolloutBuffer):
    """
    Rollout buffer that also stores rendered frames, speeds, and info dicts
    for post-rollout VLM reward computation.
    """
    ...
    def add(self, obs, action, reward, episode_start, value, log_prob, infos, render_arrays, speeds):
        ...
        self.rewards[self.pos] = np.array(reward).copy()  # Will be updated with VLM-weighted rewards
    ...
    def clear_render_arrays(self):
        """
        Clear render_arrays to free memory after VLM reward computation.
        """
        self.render_arrays = np.zeros_like(self.render_arrays)


class VLMReplayBuffer(VLMRolloutBuffer):
    """
    Replay buffer for VLM reward computation, extending VLMRolloutBuffer.
