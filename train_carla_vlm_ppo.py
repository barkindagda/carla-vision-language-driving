...
# ============================================================================
# HELPER FUNCTION FOR VLM WEIGHTS
# ============================================================================
...
class VLMSimpleCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # Clear CUDA cache and log memory every 100 steps for all GPUs
        if self.num_timesteps % 100 == 0 and torch.cuda.is_available():
            for gpu_id in range(torch.cuda.device_count()):
                torch.cuda.set_device(gpu_id)
                torch.cuda.empty_cache()
                allocated = torch.cuda.memory_allocated(gpu_id) / 1024**2
                max_allocated = torch.cuda.max_memory_allocated(gpu_id) / 1024**2
                print(f"Step {self.num_timesteps}, GPU {gpu_id}: Allocated {allocated:.2f} MB, Max {max_allocated:.2f} MB")
                if wandb.run:
                    wandb.log({
                        f"GPU_{gpu_id}/allocated_MB": allocated,
                        f"GPU_{gpu_id}/max_allocated_MB": max_allocated
        ...
        if self.locals['dones'][0]:
            ...
            self.current_episode_reward_sum = 0
            self.current_episode_step_count = 0
            # Clear CUDA cache for all GPUs after episode
...
def main():
    ...
    # Distribute policy network across GPUs
    if torch.cuda.device_count() > 1:
        model.policy = torch.nn.DataParallel(model.policy, device_ids=[0, 1, 2, 3])
        print("✅ Distributed policy network across 4 GPUs")
    ...
    if torch.cuda.is_available():
        for gpu_id in range(torch.cuda.device_count()):
            torch.cuda.set_device(gpu_id)
            torch.cuda.empty_cache()
            print(f"GPU {gpu_id}: Cache cleared before training")
    ...
    try:
        ...
        if torch.cuda.is_available():
            for gpu_id in range(torch.cuda.device_count()):
                torch.cuda.set_device(gpu_id)
                torch.cuda.empty_cache()
                print(f"GPU {gpu_id}: Cache cleared on interrupt")
        ...
        if torch.cuda.is_available():
            for gpu_id in range(torch.cuda.device_count()):
                torch.cuda.set_device(gpu_id)
                torch.cuda.empty_cache()
                print(f"GPU {gpu_id}: Cache cleared on error")
        ...
        if torch.cuda.is_available():
            for gpu_id in range(torch.cuda.device_count()):
                torch.cuda.set_device(gpu_id)
                torch.cuda.empty_cache()
                print(f"GPU {gpu_id}: Cache cleared at end")
