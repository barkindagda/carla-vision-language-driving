...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, vlm_frames=3, use_symbolic_rewards=False, 
        ...
        self.smoothed_vlm_weights = self.default_weights.copy()
        self.prev_dist_to_target = None # Initialize as None
        self.weight_smoothing_factor = 1 # Alpha. Higher means more responsive, lower is smoother. 0.8 is a good start.
    ...
    def save_current_frame(self):
        ...
        if self.front_camera is not None and self.frame_save_dir is not None:
            ...
            try:
                ...
                # Keep last 10 frames for VLM selection
    ...
    def get_vlm_selected_frames(self):
        ...
        Returns:
            list: [first, middle, last] frame paths
        """
        if len(self.frame_buffer) < 10:
            # Not enough frames, fallback to last available
            return self.frame_buffer[-3:] if len(self.frame_buffer) >= 3 else self.frame_buffer
        first = self.frame_buffer[0]
        middle = self.frame_buffer[4]  # 5th frame (index 4)
        last = self.frame_buffer[9]
    ...
    def reset(self, seed=None, options=None):
        ...
        ### NEW ###
        # Reset the distance tracking for the progress reward
    ...
    def step(self, action=None):
        ...
        if not self.use_symbolic_rewards:
            ...
            scale_factor =1 #Tune this (e.g., 5 to 20) based on desired magnitude
            c1 = c1 * scale_factor
            c2 = c2 * scale_factor
            c3 = c3 * scale_factor
    ...
    def get_vlm_weights(self):
        """
        Get reward component weights from VLM controller, applying exponential moving average (EMA)
        to smooth the output and stabilize training.
        
        Returns:
            dict: A dictionary of smoothed weights for each reward component.
        """
        # 1. Return default weights immediately if VLM weights are disabled
        if not self.use_vlm_weights:
            return self.default_weights
            
        # 2. Check if it's time for an update from the VLM
        should_update = (self.timestep % self.vlm_controller.update_frequency == 0)

        # 3. If it's time to update, get new weights and apply smoothing
        if should_update and hasattr(self, "vlm_controller") and hasattr(self.vlm_controller, "get_reward_weights"):
            
            # This block attempts to get new "raw" weights from the VLM
            try:
                # Use selected frames for VLM
                ...
                if len(selected_frames) == 3:
                    ...
                    # Get raw (unsmoothed) weights from the VLM controller
                    vlm_output = self.vlm_controller.get_reward_weights(vehicle_state, selected_frames)
                    
                    # Validate the VLM output before using it
                    if vlm_output and all(k in vlm_output for k in ["safety_weight", "comfort_weight", "efficiency_weight"]):
                        raw_weights = {
                            "w1": vlm_output["safety_weight"],
                            "w2": vlm_output["comfort_weight"],
                            "w3": vlm_output["efficiency_weight"]
                        }

                        # --- EMA SMOOTHING LOGIC ---
                        alpha = self.weight_smoothing_factor
                        
                        # Update each smoothed weight based on the new raw weight and the previous smoothed value
                        self.smoothed_vlm_weights["w1"] = alpha * raw_weights["w1"] + (1 - alpha) * self.smoothed_vlm_weights["w1"]
                        self.smoothed_vlm_weights["w2"] = alpha * raw_weights["w2"] + (1 - alpha) * self.smoothed_vlm_weights["w2"]
                        self.smoothed_vlm_weights["w3"] = alpha * raw_weights["w3"] + (1 - alpha) * self.smoothed_vlm_weights["w3"]

                        # # Re-normalize the smoothed weights so they sum to 1
                        # total_weight = sum(self.smoothed_vlm_weights.values())
                        # if total_weight > 0:
                        #     for key in self.smoothed_vlm_weights:
                        #         self.smoothed_vlm_weights[key] /= total_weight
                        
                        if self.vlm_controller.verbose:
                            print(f"Timestep {self.timestep}: Raw VLM weights: {raw_weights}")
                            print(f"Timestep {self.timestep}: Smoothed VLM weights: {self.smoothed_vlm_weights}")

                    else:
                        if self.vlm_controller.verbose:
                            print(f"VLM output missing keys. Using previous smoothed weights.")
                else:
                    if self.vlm_controller.verbose:
                        print(f"Not enough selected frames for VLM update. Using previous smoothed weights.")
            
            except Exception as e:
                print(f"Error getting VLM weights: {e}. Using previous smoothed weights.")
        
        # 4. Always return the latest smoothed weights.
        # On non-update steps, this returns the values from the last successful update.
        return self.smoothed_vlm_weights
    ...
    #     """
    #     Update reward statistics using EWMA and normalize reward.
        
    #     Args:
    #         reward: Raw reward value
            
    #     Returns:
    #         Normalized or original reward value
    #     """
    #     # Hyperparameter for exponential decay
    #     self.alpha = 0.01  # Controls how much weight recent rewards have
    ...
    #     self.reward_running_mean += self.alpha * delta
    #     delta2 = reward - self.reward_running_mean
    #     self.reward_running_var = (1 - self.alpha) * (self.reward_running_var + self.alpha * delta * delta2)
        
    #     # Compute standard deviation
    #     self.reward_running_std = max(np.sqrt(self.reward_running_var), 0.1)
        
    #     # Normalize if enabled and enough samples
    #     if self.normalize_rewards and self.reward_count >= 50:
    #         normalized = (reward - self.reward_running_mean) / self.reward_running_std
    #         return float(np.clip(normalized, -1.0, 5.0))
    ...
    def process_reward(self, reward):
        ...
        if self.normalize_rewards and self.reward_count >= 100:
            ...
            normalized = (reward - self.reward_running_mean) / self.reward_running_std
            return float(np.clip(normalized, -0.05,5))###########################
