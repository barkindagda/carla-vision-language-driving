...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, vlm_frames=3, weather=None):
        super().__init__()
        ...
        # Store only weather settings
        self.weather = weather
        
        # Apply weather setting if specified
    ...
    def reset(self, seed=None, options=None):
        ...
        # Handle weather changes only
        if options is not None:
            if 'weather' in options:
                self.set_weather(options['weather'])
        else:
            # Randomly change weather each episode
            self.set_weather()
    ...
    def set_weather(self, weather_preset=None):
        ...
        # Ensure we have a valid WeatherParameters object
        if isinstance(weather_preset, str):
            weather_preset = getattr(carla.WeatherParameters, weather_preset)
    
        self.world.set_weather(weather_preset)

    def set_town(self, town_name=None):
        """Load a new town in the CARLA world"""
        if town_name is None:
            # Available town names
            town_names = ['Town01', 'Town02', 'Town03', 'Town04', 'Town05', 'Town06', 'Town07', 'Town10HD']
            town_name = random.choice(town_names)
        
        # Load the new map
        self.world = self.client.load_world(town_name)
        
        # Re-configure world settings after map change
        self.settings = self.world.get_settings()
        self.settings.fixed_delta_seconds = 0.10
        self.settings.synchronous_mode = True
        self.world.apply_settings(self.settings)
        
        # Update traffic manager for new world
        self.traffic_manager = self.client.get_trafficmanager()
        self.traffic_manager.set_synchronous_mode(True)