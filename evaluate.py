...
from environment.carla_env_eval import CarlaEnv
...
# Remove the town argument since we want to keep it fixed
parser.add_argument("--weather", type=str, 
                   choices=['ClearNoon', 'CloudyNoon', 'WetNoon', 'WetCloudyNoon', 'MidRainyNoon',
                           'HardRainNoon', 'SoftRainNoon', 'ClearSunset', 'CloudySunset', 'WetSunset',
                           'WetCloudySunset', 'MidRainSunset', 'HardRainSunset', 'SoftRainSunset'],
                   default=None, help="Weather preset to use")
...
try:
    ...
    # Store current town name (assuming Town02 is fixed)
    current_town = "Town02"
...
def run_eval(env, model, model_path, record_video=False, episodes=10):
    ...
    # Define CSV columns based on CarlaEnv info, adding pedestrian_detected
    columns = [
        "model_id", "episode", "step", "throttle", "steer", "vehicle_location_x", "vehicle_location_y",
        "reward", "distance_to_goal", "distance_to_ego_target", "pedestrian_distance", "speed_ms",
        "safety_reward", "progress_reward", "smoothness_reward", "collision_penalty",
        "pedestrian_detected", "done_reason", "routes_completed", "town", "weather"  # Added town and weather
    ]
    df = pd.DataFrame(columns=columns)

    # Lists to track stopping distances and pedestrian detection steps
    stopping_distances = []
    pedestrian_detection_steps = {i: [] for i in range(episodes)}  # Dict to store steps per episode
    ...
    while episode_idx < episodes:
        ...
        while not done:
            try:
                ...
                # Handle 4-element step return (CarlaEnv returns obs, reward, done, done, info)
                ...
                if route_ind is not None and route is not None:
                    print(f"Step {step_count}: route_ind={route_ind}, route_length={len(route)}, current_waypoint={route[route_ind]}")
                ...
                # Calculate distance to ego_target
                ego_target = env.envs[0].ego_target
                distance_to_ego_target = np.linalg.norm(
                    np.array([location.x, location.y]) - np.array([ego_target.x, ego_target.y])
                )

                # Check pedestrian detection
                pedestrian_detected = info_dict.get("pedestrian_detected", False)
                if pedestrian_detected:
                    pedestrian_detection_steps[episode_idx].append(step_count)

                # Check for stopping event (speed < 0.1 m/s and pedestrian detected)
                if pedestrian_detected and info_dict.get("speed_ms", 0) < 0.1:
                    stopping_distances.append(info_dict.get("pedestrian_distance", 0))
                ...
                if done or truncated:
                    if len(info_dict.get("collision_hist", [])) > 0:
                        ...
                        done_reason = "unknown"
                        print(f"Warning: Episode terminated with unknown reason, info={info_dict}")
                ...
                new_row = pd.DataFrame([[
                    ...
                    current_town,  # Use fixed town name
                    args['weather'] if args['weather'] else "Default"  # Add weather with default fallback
                ]], columns=columns)
    ...
    avg_stopping_distance = np.mean(stopping_distances) if stopping_distances else 0.0
    num_stopping_events = len(stopping_distances)
    ...
    summary = {
        ...
        "avg_distance_to_ego_target": df["distance_to_ego_target"].mean(),
        "avg_speed_ms": df["speed_ms"].mean(),
        "collisions": len(df[df["done_reason"] == "collision"]),
        "timeouts": len(df[df["done_reason"] == "timeout"]),
        "successes": len(df[df["done_reason"] == "success"]),
        "avg_stopping_distance": avg_stopping_distance,
        "num_stopping_events": num_stopping_events,
        "pedestrian_detection_steps": pedestrian_detection_steps
    }
    print("Evaluation Summary:")
    for key, value in summary.items():
        if key == "pedestrian_detection_steps":
            print(f"{key}:")
            for ep, steps in value.items():
                if steps:
                    print(f"  Episode {ep}: Steps {steps}")
                else:
                    print(f"  Episode {ep}: No pedestrian detected")
        else:
            print(f"{key}: {value}")
