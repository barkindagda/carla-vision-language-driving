...
TOTAL_TIMESTEPS = 350_000  # Already set, but ensure consistency
CHECKPOINT_INTERVAL = 20000
SEED = 42
EFFICINCY_PRIORITY = True
EXTRACTOR_CHOICE = "DEFULT_CNN"  # Options: "DEFAULT_CNN", "CLIP_SEG_SPATIAL", "PEDESTRIAN_ATTENTIVE"
...
# Linear learning rate schedule (match old method)
INITIAL_LR = 0.01  # Match old Config
FINAL_LR = 0.0005
linear_schedule = get_linear_fn(INITIAL_LR, FINAL_LR, 1.0)

PPO_CONFIG = {
    "learning_rate": linear_schedule,  # Use linear schedule
    "n_steps": 2048,                  # Match old method
    "batch_size": 100,                 # Match old method
    "n_epochs": 10,                   # Default SB3 is 10, old method may use 10
    "gamma": 0.99,
    "gae_lambda": 0.95,
    "clip_range": 0.2,
    "ent_coef": 0.01,
    ...
    "max_grad_norm": 0.5,
    "verbose": 0,
    "net_arch_mlp_head": [dict(pi=[256, 128], vf=[256, 128])],  # Match old method
}
ENV_RENDER_MODE = None
ENV_USE_VLM_WEIGHTS = True
ENV_USE_VLM_ACTIONS = False
ENV_NORMALIZE_REWARDS = True  # Disable reward normalization to match old method
VLM_VERBOSE = True
...
WANDB_PROJECT = "carla-ppo"
WANDB_ENTITY = None

# --- New: Path to pre-trained model (set to your checkpoint zip file) ---
PRETRAINED_MODEL_PATH = None  # e.g. './experiments/your_old_exp/models/model_step_200000.zip'

# --- Helper for VLM weights log ---
...
def main():
    global vlm_controller_instance
    print(f"🚗 CARLA VLM-Enhanced PPO Training (Pretrained Start)\n" + "="*70)
    ...
    vlm_controller_instance = VLMController(
        ...
        max_new_tokens=VLM_MAX_TOKENS, verbose=VLM_VERBOSE, efficiency_priority=EFFICINCY_PRIORITY
    )
    ...
    if PRETRAINED_MODEL_PATH and os.path.exists(PRETRAINED_MODEL_PATH):
        print(f"🔄 Loading pre-trained PPO model from: {PRETRAINED_MODEL_PATH}")
        model = PPO.load(PRETRAINED_MODEL_PATH, env=vec_env, tensorboard_log=LOG_DIR, policy_kwargs=policy_kwargs, seed=SEED, **model_hyperparams)
    else:
        print("🆕 Training from scratch (no pre-trained model found or specified).")
